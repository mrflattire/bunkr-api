...
def parse_arguments():
    """Parse command line arguments for the optimized functional tester."""
    parser = argparse.ArgumentParser(description="Optimized Functional Asynchronous CDN Token Minter Tester.")
    parser.add_argument(
        '-i', '--input',
        type=str,
        required=True,
...
async def test_main():
    ...
    progress_bar = Progress(
        TextColumn("[bold cyan][*][/bold cyan] Optimized Async Minting..."),
    ...
    async def worker(session, file_item, pb, tid, idx):
        # Pool Warmup: Stagger only the initialization of the first 5 workers.
        # This gives the underlying sockets room to negotiate TLS cleanly.
        if idx < 5:
    ...
    test_output_path = input_json_path.parent / f"optimized_verify_{input_json_path.name}"
    ...
    console.print(f"[bold green][+][/bold green] Complete! Successfully validated tokens for [bold cyan]{minted_count}/{total_files}[/bold cyan] assets.")
