...
def download_assets():
    ...
    try:
        ...
        # Label any mid-download worker rows as interrupted instead of
        # leaving them frozen on a stale percentage. This writes directly
        # rather than going through update_worker_slot(), since that helper
        # deliberately no-ops once shutdown_event is set -- here we WANT
        # one last write, we just want it to be this one, on purpose, from
        # the main thread only, after every worker thread is confirmed dead.
        for slot_id, _ in interrupted_slots:
            row = TERMINAL_ROWS - workers + slot_id + 1
            sys.stdout.write(f"\033[{row};1H\033[2K[Worker {slot_id + 1}] Interrupted")
        sys.stdout.flush()
