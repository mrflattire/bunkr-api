...
def poll_mpv_status(ipc_path: str, stop_event: threading.Event, total_tracks: int):
    """
    Polls the MPV JSON IPC interface in a background thread and updates a
    real-time Rich progress panel dynamically.
    """
    # 1. Wait briefly for MPV to initialize and host the IPC channel
    connected = False
    sock_file = None
    ...
    try:
        with Live(Panel(progress_bar, title="[bold green]Live Stream Player Status[/bold green]", border_style="green"), refresh_per_second=5):
            while not stop_event.is_set():
                name = query("media-title")
                pos = query("time-pos")
                dur = query("duration")
                percent = query("percent-pos")
                cache = query("demuxer-cache-duration")
                playlist_pos = query("playlist-pos")  # 0-indexed track sequence
                ...
                if name:
                    ...
                    display_title = (display_title[:32] + '...') if len(display_title) > 35 else display_title
                else:
                    display_title = "Buffering stream..."

                # Format human readable properties safely
                time_pos_str = fmt_time(pos)
                duration_str = fmt_time(dur)
                percent_val = int(percent) if percent is not None else 0
                cache_val = f"{cache:.1f}" if cache is not None else "0.0"

                progress_bar.update(
                    track_task,
                    description=f"Playing: {display_title}",
                    completed=percent_val,
                    time_pos=time_pos_str,
                    duration=duration_str,
                    cache_duration=cache_val
                )
...
def play_playlist_mode(playback_queue: list):
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".m3u", mode="w", encoding="utf-8") as tmp_playlist:
            ...
            for idx, item in playback_queue:
                # Prioritize original filename, fallback to title
    ...
    try:
        ...
        # Block until the window exits or is manual closed
        proc.wait()
