...
def test_register_album_creates_album_and_assets(temp_db):
    ...
    album_id, new_count, updated_count = temp_db.register_album_from_json(sample_data)
...
def test_register_album_true_file_id_falls_back_to_slug_id(temp_db):
    ...
    album_id, _new_count, _updated_count = temp_db.register_album_from_json(sample_data)
...
def test_register_album_upsert_updates_existing_album_not_duplicated(temp_db):
    ...
    first_id, first_new, first_updated = temp_db.register_album_from_json({
        "selected_album": {**base_album, "aggregate_size": "10 MB"},
        "files_found": [{"href": "https://link.com/f/a", "title": "a.mp4"}],
    })
    second_id, second_new, second_updated = temp_db.register_album_from_json({
    ...
    assert first_new == 1 and first_updated == 0
    # Second pass: "a.mp4" already existed (updated), "b.mp4" is new.
    assert second_new == 1 and second_updated == 1
...
def test_register_album_upsert_updates_existing_asset_not_duplicated(temp_db):
    ...
    album_id, _new_count, _updated_count = temp_db.register_album_from_json(first_pass)
...
def _register_asset(temp_db, href, expiry_ts, true_file_id=1, album_title="Refresh Album", album_index=1):
    ...
    album_id, _new_count, _updated_count = temp_db.register_album_from_json(data)
...
# ============================================================
# get_staged_assets / get_failed_assets / sync_album_staged_flag
# ============================================================


def _stage_whole_album(temp_db, album_id):
    """Mirrors inspector.py's toggle_staging(target='album'): sets
    is_staged on both the album row and every asset in it.
    """
    with temp_db.connection() as conn:
...
def test_get_staged_assets_ignores_album_level_flag(temp_db):
    """Regression test: an album-staged asset that's already had its own
    is_staged cleared (e.g. after a completed download) must NOT reappear
    just because its parent album's is_staged flag is still 1.
    """
    album_id, _, _ = temp_db.register_album_from_json({
        "selected_album": {"title": "Album", "album_index_number": 1},
        "files_found": [{"href": "https://x/1", "title": "a.mp4"}],
    })
    _stage_whole_album(temp_db, album_id)
    asset_id = temp_db.get_album_assets(album_id)[0]["id"]

    with temp_db.connection() as conn:
        conn.execute("UPDATE assets SET is_staged = 0 WHERE id = ?", (asset_id,))

    staged = temp_db.get_staged_assets()
    assert asset_id not in {r["id"] for r in staged}


def test_get_staged_assets_includes_album_title(temp_db):
    album_id, _, _ = temp_db.register_album_from_json({
        "selected_album": {"title": "Titled Album", "album_index_number": 1},
        "files_found": [{"href": "https://x/1", "title": "a.mp4"}],
    })
    _stage_whole_album(temp_db, album_id)

    staged = temp_db.get_staged_assets()
    assert staged[0]["album_title"] == "Titled Album"


def test_get_failed_assets_returns_only_failed(temp_db):
    album_id, _, _ = temp_db.register_album_from_json({
        "selected_album": {"title": "Album", "album_index_number": 1},
        "files_found": [
            {"href": "https://x/1", "title": "ok.mp4"},
            {"href": "https://x/2", "title": "broken.mp4"},
        ],
    })
    assets = temp_db.get_album_assets(album_id)
    temp_db.update_download_status(assets[1]["id"], "FAILED", error="timeout")

    failed = temp_db.get_failed_assets()
    assert len(failed) == 1
    assert failed[0]["id"] == assets[1]["id"]
    assert failed[0]["album_title"] == "Album"


def test_sync_album_staged_flag_clears_when_no_assets_remain_staged(temp_db):
    album_id, _, _ = temp_db.register_album_from_json({
        "selected_album": {"title": "Album", "album_index_number": 1},
        "files_found": [{"href": "https://x/1", "title": "a.mp4"}],
    })
    ...
    temp_db.sync_album_staged_flag(album_id)

    assert temp_db.get_album(album_id)["is_staged"] == 0


def test_sync_album_staged_flag_leaves_flag_set_while_siblings_staged(temp_db):
    album_id, _, _ = temp_db.register_album_from_json({
        "selected_album": {"title": "Album", "album_index_number": 1},
        "files_found": [
            {"href": "https://x/1", "title": "a.mp4"},
            {"href": "https://x/2", "title": "b.mp4"},
        ],
    })
    _stage_whole_album(temp_db, album_id)
    asset_a_id = temp_db.get_album_assets(album_id)[0]["id"]
    ...
    assert temp_db.get_album(album_id)["is_staged"] == 1