...
class PlayerEngine:
    ...
    def poll_mpv_status(self, ipc_path: str, stop_event: threading.Event, total_tracks: int):
        ...
        track_task = progress_bar.add_task(
            "Buffering...", total=100, time_pos="00:00", duration="00:00", cache_duration="0.0"
        )
        ...
        with Live(
            Panel(
                progress_bar,
                title="[bold green]Live Stream Player Status[/bold green]",
                border_style="green",
            ),
            ...
            while not stop_event.is_set():
                try:
                    ...
                    if data.get("event") == "property-change":
                        ...
                        t_prefix = (
                            f"[{int(state['playlist-pos'] or 0) + 1}/{total_tracks}] "
                            if state["playlist-pos"] is not None
                            else ""
                        )
                        ...
                        progress_bar.update(
                            ...
                            cache_duration=f"{cache_val:.1f}" if cache_val is not None else "0.0",
                        )
    ...
    async def resolve_tokens_async(self, assets):
        """Parallel token refresh for streaming with NULL safety and fixed ID mapping."""
        ...
        console.print(
            f"[bold yellow][*][/bold yellow] [Escape Hatch] Concurrent batch-refresh triggered for [cyan]{len(needed)}[/cyan] asset(s)..."
        )
        ...
        async def worker(session, a):
            async with sem:
                try:
                    # Use 'id' explicitly to avoid KeyError
                    ...
                    safe_title = (a.get("title") or "unknown")[:20]
                    console.print(f"[dim red]Minting failed for {safe_title}: {e}[/dim red]")

        async with AsyncSession(impersonate="chrome") as session:
            await asyncio.gather(*[worker(session, a) for a in needed])

    async def play_mpv(self, playback_queue):
        """Assembles M3U and launches MPV with IPC polling. Restored Feedback logs."""
        console.print(
        ...
        try:
            ...
            console.print(
                f"[bold red][-][/bold red] Failed to generate temporary play-queue file: {e}"
            )
            return

        ipc = (
            rf"\\.\pipe\mpv_{os.getpid()}"
            if os.name == "nt"
            else os.path.join(tempfile.gettempdir(), f"mpv_{os.getpid()}")
        )
        stop_event = threading.Event()
        poll_thread = threading.Thread(
            target=self.poll_mpv_status, args=(ipc, stop_event, len(playback_queue)), daemon=True
        )

        console.print("[bold green][*][/bold green] Launching MPV engine with IPC control...")

        try:
            # Wrap blocking subprocess in thread to keep loop alive
            proc = await asyncio.to_thread(
                subprocess.Popen,
                ["mpv", "--force-window", f"--input-ipc-server={ipc}", "--idle=once", p_path],
            ...
            console.print(
                "[bold red][-][/bold red] Error: 'mpv' executable not found on system PATH."
            )
    ...
    async def play_vlc(self, playback_queue):
        """VLC handoff logic. Restored Feedback logs."""
        console.print(
            f"\n[bold green][*][/bold green] Assembling Playlist ([cyan]{len(playback_queue)} tracks[/cyan])..."
        )
        ...
        console.print(
            "[bold green][*][/bold green] Streaming via VLC... close the window to return."
        )
        try:
            await asyncio.to_thread(subprocess.run, [vlc, p_path], check=False)
        finally:
            if os.path.exists(p_path):
                os.unlink(p_path)
            console.print("[bold green][+][/bold green] Player session closed safely.")
...
def prompt_for_inputs(db):
    ...
    try:
        raw = Prompt.ask(
            "[bold cyan][?][/bold cyan] Choose an album by number, drop a fresh JSON path, or 'q' to exit"
        ).strip()
    except KeyboardInterrupt:
    ...
    if raw.lower() == "staged":
        player = Prompt.ask(
            "[bold cyan][?][/bold cyan] Select Media Player Engine",
            choices=["mpv", "vlc"],
            default="mpv",
        )
    ...
    if raw.isdigit():
        ...
        if db_albums and 1 <= num_val <= len(db_albums):
            ...
            console.print(
                f"[*] Resolved selection to: [yellow]{db_albums[num_val - 1]['title']}[/yellow] (DB ID: {db_id})"
            )
        else:
            db_id = num_val
    else:
    ...
    selection = Prompt.ask(
        "[bold cyan][?][/bold cyan] Enter item index, list, or range [dim](or Press Enter for ALL)[/dim]"
    ).strip()
    if not selection:
        selection = "all"
    player = Prompt.ask(
...
async def run_player_logic(args_db_id=None, args_number=None, args_player=None, args_staged=False):
    """Internal orchestration to handle both API and CLI calls."""
    ...
    db_id = args_db_id
    selection_arg = args_number
    player_choice = args_player
    run_staged = args_staged

    if not db_id and not run_staged and not selection_arg:
    ...
    if db_backed_assets:
        await engine.resolve_tokens_async(db_backed_assets)

    # Format playback queue
    queue = []
    for i in indices:
        item = files_list[i - 1]
...
def main():
    ...
    if loop_f:
        asyncio.run(
            run_player_logic(args.db_id, args.number, args.player, args.staged), loop_factory=loop_f
        )
    else:
        asyncio.run(run_player_logic(args.db_id, args.number, args.player, args.staged))
